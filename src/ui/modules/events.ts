...
export function setupEventsControlsCustom(
  ...
  let resultsDiv = document.getElementById('events-results-dropdown') as HTMLElement;
  if (!resultsDiv) {
  ...
  const searchInput = eventsContainer.querySelector('#events-search') as HTMLInputElement;
  const goToBtn = eventsContainer.querySelector('#btn-go-to-event') as HTMLButtonElement;
  const statusDiv = eventsContainer.querySelector('#events-status') as HTMLElement;
