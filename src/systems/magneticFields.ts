 ...
 */
export function createSunMagneticFieldBasic(_sunMesh: THREE.Mesh) {
  const group = new THREE.Group();
  ...
  const loadTexturePixelData = (url: string): Promise<ImageData> => {
  ...
  const generateFields = async () => {
    ...
    const getVoxelKey = (p: THREE.Vector3) => {
 ...
 * @returns {THREE.Group} The group containing the field lines
 */
export function createSunMagneticField(_sunMesh: THREE.Mesh) {
  const group = new THREE.Group();
  group.name = 'MagneticField';
